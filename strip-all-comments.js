...
htmlFiles.forEach(f => {
  try {
    ...
    // First strip CSS comments inside <style> tags (before HTML comments to avoid false positives)
    let cleaned = content.replace(/(<style[^>]*>)([\s\S]*?)(<\/style>)/gi, (m, openTag, inner, closeTag) => {
      return openTag + removeCssComments(inner) + closeTag;
    });
    // Strip JS comments inside <script> tags (only inline scripts, before HTML comments)
    cleaned = cleaned.replace(/(<script(?![^>]*src)[^>]*>)([\s\S]*?)(<\/script>)/gi, (m, openTag, inner, closeTag) => {
      return openTag + removeJsComments(inner) + closeTag;
    });
    // Finally strip HTML comments (after script/style processing)
