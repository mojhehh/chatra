...
2. Enable 2-Step Verification on the Google account used for Firebase Console access (https://myaccount.google.com/signinoptions/two-step-verification)
3. For per-user 2FA on Firebase Auth accounts, implement Firebase Multi-Factor Authentication (https://firebase.google.com/docs/auth/web/multi-factor)
4. Limit access to credentials to platform owner only
5. Document recovery email and phone number separately (in vault, not here)
...
### Overview
Device fingerprinting is used to prevent ban evasion. Both types require **explicit opt-in** unless a documented security exemption applies:

1. **Basic Device Fingerprint** (non-canvas): Opt-in required; may be enabled via security exemption
2. **Canvas Fingerprint**: Opt-in required; more unique and regulated under CCPA/GDPR

### Security Exemption Checklist (Required for Default Collection)
If enabling fingerprinting without opt-in for security purposes, document the following:
- [ ] **Necessity & Proportionality Analysis**: Document why fingerprinting is necessary and proportionate to the security threat
- [ ] **Less-Intrusive Alternatives Review**: Document why less-intrusive methods (IP-based, session-based) are insufficient
- [ ] **GDPR Balancing Test / Legal Review**: Conduct legitimate interest assessment (LIA) with legal counsel
- [ ] **Purpose Limitation**: Confirm fingerprinting is used ONLY for security (no tracking, profiling, or ad targeting)
- [ ] **Transparent Disclosure**: Update privacy policy with clear description of techniques and purposes
- [ ] **Easy Opt-Out**: Ensure users can opt-out at any time with immediate effect

### Regional Compliance Notes
- **GDPR (EU/EEA)**: Requires consent or documented legitimate interest with balancing test; DPA notification may be required
- **ePrivacy Directive**: Device access generally requires prior consent unless strictly necessary
- **CCPA (California)**: Disclose in "Sale/Share" notices; honor opt-out requests
- **LGPD (Brazil)**: Requires legal basis; document necessity and user notification

### Privacy Policy Requirements
Update privacy policy to disclose:
- Fingerprinting techniques used (browser attributes, canvas, etc.)
- Purposes (ban evasion prevention only)
- User consent/withdrawal rights and how to exercise them
