...
# Also try to find any remaining weird sequences
import re
# Find sequences that look like mojibake (multiple high-byte chars)
weird = re.findall(b'[\xc0-\xff]{3,8}', content[:5000])
if weird:
    print(f"\nPotentially remaining mojibake sequences (first 5KB): {set(weird)}")
