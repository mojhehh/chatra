(() => {
  ...
  // verify-email-worker.js
  var require_verify_email_worker = __commonJS({
    "verify-email-worker.js"() {
      ...
      __name(rateAllowed, "rateAllowed");
      ...
      __name(importPublicKeyFromCert, "importPublicKeyFromCert");
      function extractPublicKeyFromCert(certDer) {
        ...
        for (let i = 0; i < cert.length - rsaOid.length - 50; i++) {
          ...
          if (match) {
            let seqStart = i - 4;
            while (seqStart > 0 && cert[seqStart] !== 48) seqStart--;
            if (cert[seqStart] === 48) {
      ...
      __name(extractPublicKeyFromCert, "extractPublicKeyFromCert");
      ...
      var FIREBASE_PROJECT_ID = "chat-app-710f0";
      async function handleRequest(request) {
        ...
        if (request.method === "POST" && url.pathname === "/send-verification") {
          try {
            ...
            if (!await rateAllowed(ip, "send-verification", 10, 60 * 60 * 1e3, kvBinding)) {
            ...
            // Validate email against server-side source (token email)
            if (verifiedUser.email && verifiedUser.email !== email) {
              return respond("Unauthorized: Email mismatch with verified token", { status: 401 });
            }
            ...
            const dbUrl = (typeof FIREBASE_DB_URL !== "undefined" && FIREBASE_DB_URL) ? FIREBASE_DB_URL : FIREBASE_DB_URL_FROM_ENV();
            if (!dbUrl) return respond("FIREBASE_DB_URL not configured", { status: 500 });
            const tokenObj = { uid, email, createdAt: now, expiresAt };
            const tokenPut = await fetch(`${dbUrl}/recoveryTokens/${tokenId}.json`, {
              method: "PUT",
              headers: { "Content-Type": "application/json", "Authorization": `Bearer ${firebaseAccess}` },
        ...
        if (request.method === "POST" && url.pathname === "/verify-token") {
          try {
            const ip = request.headers.get("CF-Connecting-IP") || request.headers.get("X-Forwarded-For") || "unknown";
            if (!await rateAllowed(ip, "verify-token", 60, 60 * 60 * 1e3, kvBinding)) {
              return respond("Too many requests", { status: 429 });
            }
            const body = await request.json();
            ...
            const dbUrl = (typeof FIREBASE_DB_URL !== "undefined" && FIREBASE_DB_URL) ? FIREBASE_DB_URL : FIREBASE_DB_URL_FROM_ENV();
            if (!dbUrl) return respond("FIREBASE_DB_URL not configured", { status: 500 });
            const tokRes = await fetch(`${dbUrl}/recoveryTokens/${tokenId}.json`, {
              headers: { "Authorization": `Bearer ${firebaseAccess}` }
            });
            ...
            return respond(JSON.stringify({ ok: true, email: tok.email, uid: tok.uid }), { status: 200, headers: { "Content-Type": "application/json" } });
          } catch (e) {
        ...
        if (request.method === "POST" && url.pathname === "/reset-password") {
          try {
            const ip = request.headers.get("CF-Connecting-IP") || request.headers.get("X-Forwarded-For") || "unknown";
            if (!await rateAllowed(ip, "reset-password", 30, 60 * 60 * 1e3, kvBinding)) {
              return respond("Too many requests", { status: 429 });
            }
            const body = await request.json();
            const tokenId = body.tokenId;
            ...
            if (typeof newPassword !== "string" || newPassword.length < 12 || newPassword.length > 128) return respond("Password must be 12-128 characters", { status: 400 });
            const firebaseAccess = await getFirebaseAccessToken();
            if (!firebaseAccess) return respond("Firebase access unavailable", { status: 500 });
            const dbUrl = (typeof FIREBASE_DB_URL !== "undefined" && FIREBASE_DB_URL) ? FIREBASE_DB_URL : FIREBASE_DB_URL_FROM_ENV();
            if (!dbUrl) return respond("FIREBASE_DB_URL not configured", { status: 500 });
            ...
            // Delete the recovery token and verify deletion
            const deleteRes = await fetch(`${dbUrl}/recoveryTokens/${tokenId}.json`, { method: "DELETE", headers: { "Authorization": `Bearer ${firebaseAccess}` } });
            if (!deleteRes.ok) {
              const deleteErr = await deleteRes.text();
              console.error("Failed to delete recovery token (detail):", deleteRes.status, deleteErr);
              // Password was updated but token deletion failed - this is a security concern
              return respond("Password updated but token cleanup failed", { status: 500 });
            }
        ...
        if (request.method === "GET" && url.pathname === "/verify") {
          ...
          if (!tokenId) return respond("Missing tokenId", { status: 400 });
          const ip = request.headers.get("CF-Connecting-IP") || request.headers.get("X-Forwarded-For") || "unknown";
          if (!await rateAllowed(ip, "verify", 60, 60 * 60 * 1e3, kvBinding)) {
            return respond("Too many requests", { status: 429 });
          }
          try {
            const firebaseAccess = await getFirebaseAccessToken();
            ...
            if (!tok || !tok.uid || !tok.email) return respond("Invalid token", { status: 400 });
            if (Date.now() > (tok.expiresAt || 0)) return respond("Token expired", { status: 400 });
            const verifyRes = await fetch(`${dbUrl}/userProfiles/${tok.uid}/recoveryEmailVerified.json`, {
              method: "PUT",
              headers: { "Content-Type": "application/json", "Authorization": `Bearer ${firebaseAccess}` },
            ...
            await fetch(`${dbUrl}/recoveryTokens/${tokenId}.json`, { method: "DELETE", headers: { "Authorization": `Bearer ${firebaseAccess}` } });
        ...
        if (request.method === "POST" && url.pathname === "/check-fingerprint-ban") {
          try {
            const ip = request.headers.get("CF-Connecting-IP") || request.headers.get("X-Forwarded-For") || "unknown";
            if (!await rateAllowed(ip, "check-fp-ban", 30, 60 * 1e3, kvBinding)) {
            ...
            const banRes = await fetch(`${dbUrl}/bannedFingerprints/${safeFingerprint}.json`, {
              headers: { "Authorization": `Bearer ${firebaseAccess}` }
            });
            if (banRes.ok) {
        ...
        if (request.method === "POST" && url.pathname === "/ai") {
          try {
            ...
            const aiRateOk = await rateAllowed(clientIp, "ai", 30, 6e4, kvBinding);
            if (!aiRateOk) {
            ...
            // Sanitize username to prevent prompt injection - allow only safe characters
            const sanitizedUsername = (username || 'User').replace(/[^a-zA-Z0-9 _-]/g, '').slice(0, 32) || 'User';
            
            // Use client's history which includes system prompt, or create default
            let messages = [];
            let hasSystemPrompt = false;
            
            // Process history - keep user/assistant messages only, ignore client system prompts
            for (const msg of history.slice(-20)) {
              if (msg.role && msg.content) {
                // Skip client-provided system messages to prevent prompt injection
                if (msg.role === 'system') {
                  continue;
                }
            ...
            // Always add server-controlled system prompt (never trust client system prompts)
            messages.unshift({
              role: "system",
              content: `You are Chatra AI, the friendly assistant for the Chatra chat application. You are currently talking to a user named "${sanitizedUsername}". Be helpful and concise. Remember context from previous messages.

IMAGE FEATURES - Use when user asks for images:
- [IMAGE: query] - Show a single image (e.g., "show me a dog")
- [IMAGE_SEARCH: query] - Show multiple images (e.g., "show me pictures of cats")
- GIFs are allowed - use [IMAGE: animated gif X] for GIF requests

WEB SEARCH - Use [SEARCH: query] only when user explicitly asks to search or needs very recent info.

Keep responses compact. Don't overuse the user's name.`
            });
            ...
            if (!cereRes.ok) {
              ...
              const fallbackReply = `I'm having a brief connection issue. Could you try again in a moment?`;
              return respond(JSON.stringify({ ok: true, reply: fallbackReply }), { status: 200, headers: { "Content-Type": "application/json" } });
            }
            ...
            return respond(JSON.stringify({ ok: true, reply: replyText || JSON.stringify(cereJson) }), { status: 200, headers: { "Content-Type": "application/json" } });
          } catch (e) {
      ...
      function FIREBASE_DB_URL_FROM_ENV() {
        try {
      ...
      __name(FIREBASE_DB_URL_FROM_ENV, "FIREBASE_DB_URL_FROM_ENV");
      function FIREBASE_SA_FROM_ENV() {
        try {
          return FIREBASE_SA;
        } catch (e) {
          return null;
      ...
      __name(FIREBASE_SA_FROM_ENV, "FIREBASE_SA_FROM_ENV");
      function CEREBRAS_API_KEY_FROM_ENV() {
        try {
      ...
      __name(CEREBRAS_API_KEY_FROM_ENV, "CEREBRAS_API_KEY_FROM_ENV");
  ...
  require_verify_email_worker();
})();
//# sourceMappingURL=verify-email-worker.js.map
