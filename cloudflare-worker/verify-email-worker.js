...
async function handleRequest(request) {
  ...
  if (request.method === 'GET' && url.pathname === '/verify') {
    ...
    // Rate limit check
    const ip = request.headers.get('CF-Connecting-IP') || request.headers.get('X-Forwarded-For') || 'unknown';
    if (!await rateAllowed(ip, 'verify', 60, 60 * 60 * 1000, kvBinding)) {
      return respond('Too many requests', { status: 429 });
    }
...
const tokRes = await fetch(`${dbUrl}/recoveryTokens/${tokenId}.json`, {
  ...
  if (request.method === 'POST' && url.pathname === '/ai') {
    try {
      ...
      return respond(JSON.stringify({ ok: true, reply: replyText || JSON.stringify(cereJson) }), { status: 200, headers: { 'Content-Type': 'application/json' } });    } catch (e) {
...
function CEREBRAS_API_KEY_FROM_ENV() { try { return CEREBRAS_API_KEY; } catch(e) { return null; } }
