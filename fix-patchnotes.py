...
new_function = b'''function openPatchNotes() {
        try {
          ...
          console.error("Failed to update patchnotes DOM (patchNotesTitle or patchNotesContent):", e);
        }'''

# Find the old function boundaries
# Use more specific markers to avoid matching other empty catch blocks
start_marker = b'function openPatchNotes() {'
# Look for the closing of openPatchNotes - the function ends with } catch (e) {}
# followed by typical code patterns or whitespace
end_marker = b'} catch (e) {}'

start_idx = content.find(start_marker)
if start_idx == -1:
    print('ERROR: start_marker not found in script.js')
    exit(1)

# Find the end marker, but verify it's within a reasonable distance (function shouldn't be > 50KB)
end_idx = content.find(end_marker, start_idx)
if end_idx == -1:
    print('ERROR: end_marker not found after start_marker in script.js')
    exit(1)

# Sanity check: the function shouldn't be absurdly long
function_length = end_idx - start_idx
if function_length > 50000:  # 50KB max
    print(f'WARNING: openPatchNotes function appears to be {function_length} bytes, which seems too long.')
    print('The end_marker may have matched a different catch block. Please verify manually.')

end_idx += len(end_marker)

if end_idx <= start_idx:
    print('ERROR: end_marker found before or at start_marker')
    exit(1)
...
if os.path.exists('script.js'):
    try:
        shutil.copy2('script.js', 'script.js.bak')
        print('Backup created: script.js.bak')
    except IOError as e:
